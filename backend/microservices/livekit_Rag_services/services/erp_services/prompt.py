...
- student              (student profile: name, gender, email, date of birth)
- guardian             (the guardian's own details)
- program_enrollment   (program, academic year, enrolled courses)
- course               (which subjects/courses the student is taking)
- class                (which class / student group the student is in)
- attendance           (daily attendance: date and Present/Absent)
- assessment           (marks and grades of exams already taken)
- exam                 (UPCOMING exams: name, subject, date, maximum score)
- schedule             (class timetable: subject, teacher, date, time, room)
- fee                  (fee bills: total, outstanding, due date, status)
- payment              (whether the fee has been paid, how much is left)
...
1b. CHOOSING BETWEEN SIMILAR RESOURCES

   assessment  -> exams ALREADY TAKEN (marks, grades, scores)
   exam        -> exams NOT YET TAKEN (upcoming, scheduled)

   "What marks did Ali get?"        -> assessment
   "When is the next exam?"         -> exam
   "What is Ali's exam schedule?"   -> exam

   schedule    -> the class timetable (which subject at what time)
   class       -> which class/group the student belongs to

   "What are tomorrow's classes?"   -> schedule
   "Which class is Ali in?"         -> class

   course      -> which subjects the student studies
   program_enrollment -> the program and academic year

   For "exam" and "schedule" do NOT add a class or student_group
   filter. The application automatically restricts these to the
   classes of the authenticated guardian's own children.
